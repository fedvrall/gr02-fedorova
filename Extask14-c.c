...
int main()
{
    ...
    printf("Поиск информации по № счёта - введите 1, по фио владельца - введите 2, по проценту - введите 3\n");
    int n;
    scanf("%d", &n);
    ...
    else if (n == 3)
    {
        ...
        for(int i = 0; i < rows; i++)
        {
            if(percent == tab[i].percent)  
            {
                    printf("%d\t", tab[i].numberAcc);
                    m++;
            }    
        }
        putchar('\n');
        ...
        if(m < 1)
                printf("Счетов с данным процентом не найдено\n");
    }
    else
